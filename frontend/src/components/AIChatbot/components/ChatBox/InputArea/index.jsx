...
const InputArea = ({ inputText, setInputText, isLoading, handleSendMessage, t }) => {
    ...
    return (
        <footer className="pt-2 space-y-3 bg-slate-50/80 dark:bg-slate-900/40 border-t border-slate-200 dark:border-white/5 pb-4 backdrop-blur-md">
            <QuickActions t={t} onActionClick={onActionClick} />
            
            <div className="relative flex items-end gap-3 px-4 md:px-6">
                <div className="flex-1 bg-white dark:bg-[#0c1324] rounded-2xl ring-1 ring-slate-200 dark:ring-white/10 flex items-center px-4 py-1.5 min-h-[56px] focus-within:ring-yellow-500/50 focus-within:bg-white dark:focus-within:bg-[#0f172a] transition-all shadow-sm group">
