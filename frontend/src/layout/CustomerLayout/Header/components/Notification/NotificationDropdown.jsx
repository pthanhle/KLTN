    ...
    return (
        <div ref={wrapperRef} className="relative">
            ...
            >
                ...
                {unreadCount > 0 && (
                    <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 border-2 border-white dark:border-[#0b0f19] rounded-full"></span>
                )}
            </button>

            {/* The Dropdown Flyout Panel */}
            {isOpen && (
                <div className="absolute right-0 mt-3 w-[340px] md:w-[380px] bg-white dark:bg-[#0e121a] rounded-[24px] shadow-2xl dark:shadow-[0_15px_50px_rgba(0,0,0,0.8)] border border-slate-200/60 dark:border-white/5 overflow-hidden z-[100] transform transition-all animate-in fade-in zoom-in-95 duration-200">

                    {/* Header */}
                    <div className="flex justify-between items-center px-5 py-4 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-[#161a23]/50 backdrop-blur-md">
                        ...
                        {isLoading ? (
                            <div className="p-5 flex flex-col gap-6">
                                {[1, 2, 3].map((skeletonIndex) => (
                                    <div key={`skel-${skeletonIndex}`} className="flex items-start gap-4">
                    ...
                    {/* Footer */}
                    <div className="p-3 bg-slate-50 dark:bg-[#161a23] border-t border-slate-100 dark:border-white/5 text-center">
                        <Link to="/profile/notifications" className="text-[13px] font-bold text-slate-500 dark:text-slate-400 hover:text-yellow-600 dark:hover:text-yellow-500 transition-colors uppercase tracking-widest inline-flex items-center gap-1">
