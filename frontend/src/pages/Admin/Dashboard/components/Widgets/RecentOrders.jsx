...
export const RecentOrders = ({ orders }) => {
    ...
    return (
        <section className="bg-white dark:bg-[#141416] rounded-3xl border border-slate-200/60 dark:border-white/5 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 h-full flex flex-col">
            <header className="p-6 border-b border-slate-100 dark:border-white/5 flex justify-between items-center">
                <div>
                    <h3 className="text-xl font-bold text-slate-800 dark:text-white">{t('widget_orders_title')}</h3>
                </div>
            ...
            <article className="p-4 flex-1 flex flex-col gap-2">
                {orders && orders.length > 0 ? orders.map((order) => (
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex justify-between items-center">
                                <p className="text-xs font-semibold text-indigo-600 dark:text-indigo-400">
                                    {formatVND(order.financials?.grand_total || 0)}
                                </p>
                                <Tag color={getStatusColor(order.order_status)} className="m-0 text-[10px] uppercase font-bold border-0">
                                    {ORDER_STATUS_MAP[order.order_status] ? t(ORDER_STATUS_MAP[order.order_status].labelKey) : order.order_status}
                                </Tag>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-slate-400 space-y-2 min-h-[200px]">
                        ...
                        <span className="text-sm">{t('widget_orders_empty')}</span>
                    </div>
                )}
            </article>
