...
const SummaryCard = ({ part, selectedOptions, handleOptionSelect, errors, t }) => {
    return (
        <div className="bg-white dark:bg-[#141416] border border-slate-100 dark:border-white/5 rounded-[32px] p-6 shadow-xl shadow-slate-200/50 dark:shadow-none overflow-hidden relative group">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
                {part?.compatible_brands?.length > 0 && (
                    <span className="px-2 py-0.5 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 text-[10px] font-bold uppercase tracking-widest rounded-md border border-slate-200 dark:border-white/10">
                        {part.compatible_brands[0]} {part.compatible_brands.length > 1 && `+${part.compatible_brands.length - 1}`}
                    </span>
                )}
                {part?.category && (
            ...
            {(part?.seo_description || part?.description) && (
                    ...
                    {part.options.map((opt, idx) => (
                        <div key={idx} className="space-y-3">
                            <div className="flex items-center justify-between">
                                ...
                                {errors?.selectedOptions?.[opt.type] && (
                                    <span className="text-red-500 text-[10px] font-bold truncate ml-2">
                                        {errors.selectedOptions[opt.type].message}
                                    </span>
                                )}
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {opt.type === 'color' ? (
                                    opt.choices.map((c) => {
                                        const isSelected = selectedOptions?.[opt.type] === c.label;
                                        return (
                                            <button
                                                key={c.label}
                                                type="button"
                                                onClick={() => handleOptionSelect?.(opt.type, c.label)}
                                    ...
                                    opt.choices.map((c) => {
                                        ...
                                        const isSelected = selectedOptions?.[opt.type] === choiceLabel;
                                        return (
                                            <button
                                                key={choiceLabel}
                                                type="button"
                                                onClick={() => handleOptionSelect?.(opt.type, choiceLabel)}
