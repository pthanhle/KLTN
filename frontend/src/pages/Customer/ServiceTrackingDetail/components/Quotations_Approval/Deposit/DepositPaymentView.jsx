...
const DepositPaymentView = ({ paymentTerms, isProcessing, onConfirm, onCancel }) => {
    ...
    return (
        <div className="relative group animate-in zoom-in-95 duration-500">
            ...
            <div className="relative backdrop-blur-xl bg-white/90 dark:bg-[#141416]/90 border border-emerald-500/20 rounded-xl p-8 md:p-10 shadow-xl dark:shadow-[0_30px_60px_rgba(0,0,0,0.5)] flex flex-col">
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-800 dark:text-white uppercase">
                        {t('quo_deposit_title', 'Hoàn Tất Tạm Ứng Dịch Vụ')}
                    </h2>
                </div>

                <p className="text-slate-600 dark:text-[#a0a0a0] leading-relaxed mb-8 text-sm">
                    {t('quo_deposit_desc', 'Chữ ký của quý khách đã được ghi nhận bảo mật...')}
                </p>

                {/* Highlight Box */}
                <div className="bg-yellow-50 dark:bg-yellow-500/5 border border-yellow-200 dark:border-yellow-500/10 rounded-lg p-6 mb-8 relative overflow-hidden">
                    <span className="block text-xs font-bold uppercase tracking-widest text-yellow-600 dark:text-yellow-500 mb-2">
                        {t('quo_deposit_amount', 'Số Tiền Cọc Yêu Cầu:')}
                    </span>
                    <div className="text-4xl md:text-5xl font-black tracking-tighter text-yellow-500">
                        {formatCurrency(paymentTerms.required_deposit)} <span className="text-xl font-medium opacity-70 text-yellow-600 dark:text-yellow-500">VNĐ</span>
                    </div>
                </div>

                {/* QR Code Zone using AntD Image */}
                <div className="flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-white rounded-lg mb-10 shadow-inner group-hover/qr:shadow-md transition-all">
                    <div className="w-48 h-48 bg-white flex items-center justify-center relative border-[10px] border-white overflow-hidden shadow-sm">
                        <Image
                    ...
                    <div className="mt-4 text-slate-600 dark:text-slate-900 font-bold uppercase text-[10px] tracking-widest flex items-center gap-2">
                        <ScanLine size={16} />
                ...
                <button
                    onClick={onCancel}
                    className="mt-4 text-[10px] text-slate-500 hover:text-slate-900 dark:hover:text-white uppercase tracking-widest font-bold underline underline-offset-4 transition-colors text-center w-full"
                >
