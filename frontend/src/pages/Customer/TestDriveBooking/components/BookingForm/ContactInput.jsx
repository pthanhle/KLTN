                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <Input
                            {...field}
                            autoComplete="name"
                            placeholder={t('booking_fullname_plh', 'Nhập họ tên của bạn')}
                            className={`!h-[52px] !px-4 !rounded-2xl !bg-slate-50 dark:!bg-[#0a0a0b] !border-slate-200 dark:!border-white/10 hover:!border-yellow-500/50 focus:!border-yellow-500 !text-[15px] !font-medium !text-slate-900 dark:!text-white transition-all placeholder:!text-slate-400 ${errors.fullName ? '!border-red-500 focus:!border-red-500' : ''}`}
                        />
                    )}
                />
                {errors.fullName && <span className="text-[13px] font-medium text-red-500 block">{errors.fullName.message}</span>}
            </div>
