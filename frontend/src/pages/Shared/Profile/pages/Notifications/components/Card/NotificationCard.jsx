...
import { Settings, Wrench, Tag, ShoppingBag, Calendar, MoreVertical, Eye, EyeOff, Trash2 } from 'lucide-react';
...
const TYPE_CONFIG = {
    ...
    [NOTIFICATION_TYPES.PROMOTION]: { icon: Tag, iconClass: 'text-purple-500', bgClass: 'bg-purple-100 dark:bg-purple-900/30' },
    [NOTIFICATION_TYPES.MAINTENANCE]: { icon: Wrench, iconClass: 'text-yellow-500', bgClass: 'bg-yellow-100 dark:bg-yellow-900/30' },
};

const getConfig = (type) =>
    TYPE_CONFIG[type] || { icon: Settings, iconClass: 'text-slate-500', bgClass: 'bg-slate-100 dark:bg-slate-800' };
            ...
            <div className="relative" ref={menuRef}>
                ...
                {isMenuOpen && (
                    <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-[#1a2138] rounded-xl shadow-xl border border-slate-100 dark:border-white/5 overflow-hidden z-[60] animate-in fade-in zoom-in-95 duration-200">
                        {isUnread ? (
                            <button
                                onClick={() => { onMarkRead(notification._id); setIsMenuOpen(false); }}
                            ...
                            <button
                                onClick={() => { onMarkUnread(notification._id); setIsMenuOpen(false); }}
                                className="w-full text-left px-4 py-3 text-[13px] font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-[#23293c] transition-colors flex gap-2 items-center"
                            >
